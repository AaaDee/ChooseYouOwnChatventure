...
Currently hosted at [chooseyourownchatventure.net](https://chooseyourownchatventure.net). Due to costs associated with the API-key the page is password-protected, if you are interested in testing it please do reach out.

## Project structure and overview

The project repo contains the code for both frontend and backend. The backend uses Node and Express, and the frontend uses React. Both are written in Typescript and tested using Vitest.

There is a CI/CD pipeline using GitHub Actions [here](./.github/workflows). The project is first tested, and then pushed to an image on DockerHub. The images are watched in the production server and pulled on updates.
